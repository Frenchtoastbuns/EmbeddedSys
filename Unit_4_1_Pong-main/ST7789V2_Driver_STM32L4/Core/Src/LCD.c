...
void LCD_plotArray(uint16_t const array[], const uint8_t colour) {
  for (int i=0; i<ST7789V2_WIDTH; i++) {
    uint16_t value = array[i];
    if (value > LCD_PLOT_ARRAY_SCALE) {
      value = LCD_PLOT_ARRAY_SCALE;
    }

    uint16_t y = (uint16_t)((ST7789V2_HEIGHT - 1u) -
                 (((uint32_t)value * (ST7789V2_HEIGHT - 1u)) / LCD_PLOT_ARRAY_SCALE));
    LCD_Set_Pixel(i, y, colour);
  }
}
